...
pub fn distribute_all_winnings_handler<'info>(
    ...
    // For each player in the winning team, the remaining_accounts must be:
    // [player_1, player_1_token_account, player_2, player_2_token_account, ...]
    // This order is required for correct payout distribution.
    // ---
    require!(
    ...
    for i in 0..players_per_team {
        ...
        // Defensive: Ensure player_acc is a signer (optional, for extra safety)
        // require!(player_acc.is_signer, WagerError::InvalidPlayer);
        // Defensive: Ensure token_acc is a token account (checked by try_from below)
        let _ = Account::<TokenAccount>::try_from(token_acc)?;
    }

    // Defensive: Check for duplicate winners
    let mut seen = std::collections::HashSet::new();
    for i in 0..players_per_team {
        let winner = &ctx.remaining_accounts[i * 2];
        let winner_pubkey = winner.key();
        // Check for zero address
        require!(winner_pubkey != Pubkey::default(), WagerError::InvalidWinner);
        // Check for duplicates
        require!(seen.insert(winner_pubkey), WagerError::DuplicateWinner);
    }
    // Defensive: Check vault has enough balance for all payouts
    let total_payout = game_session.session_bet.checked_mul(2)
        .and_then(|amt| amt.checked_mul(players_per_team as u64))
        .ok_or(WagerError::TotalPotCalculationError)?;
    require!(ctx.accounts.vault_token_account.amount >= total_payout, WagerError::InsufficientVaultBalance);
    ...
    for i in 0..players_per_team {
        ...
        let total_pot = game_session.session_bet.checked_mul(players_per_team as u64)
            .and_then(|v| v.checked_mul(2))
            .ok_or(WagerError::TotalPotCalculationError)?;
        msg!("Total pot calculated: {}", total_pot);

        let winning_amount = game_session.session_bet.checked_mul(2)
